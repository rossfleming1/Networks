...
class Visualization():
    ...
    def reLocateLinks(graph):
        ...
        for edge in graph.edges():
            snode, enode = edge
            if snode in nodeposition and enode in nodeposition:
                px1, py1 = nodeposition[snode]
                px2, py2 = nodeposition[enode]
                fx, fy, tx, ty = Visualization.reLocateAlink(px1, py1, px2, py2, offset=5000)
                graph.edges[snode, enode]["pos_fnode"] = (fx, fy)
                graph.edges[snode, enode]["pos_tnode"] = (tx, ty)
            else:
                print(f"Missing node position for {snode} or {enode}")
